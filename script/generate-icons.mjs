...
const ICON_DIR = path.join(projectRoot, "src/_assets/icons");
const IMPORT_PREFIX = "@/_assets/icons";
 ...
 * src/_assets/icons에서 SVG 파일을 추가/삭제한 후 'npm run generate:icons'를 실행하세요.
 */
`;
}
...
function main() {
  ...
  const iconExports = files.map((f) => `  ${toPascal(toKebab(f))}Icon,`).join("\n");
